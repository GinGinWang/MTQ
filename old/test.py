...
def main():
    ...
    pretrained = (args.fixed) and (args.mulobj)
    dirName = create_dir(args.dataset, args.cd,pretrained, args.fixed,args.num_blocks,args.hidden_size,args.estimator,args.noise)
    ...
    # # set to Test mode
    file_dirName = create_file_dir(args.mulobj,model.name,args.dataset,args.cd, pretrained,args.fixed,args.score_normed,args.novel_ratio,args.num_blocks,args.hidden_size,args.lam,args.add,args.checkpoint)

    helper = OneClassTestHelper(dataset, model, args.score_normed, args.novel_ratio, lam = args.lam, checkpoints_dir= dirName, output_file= file_dirName,device = device, batch_size = args.batch_size, pretrained= pretrained, trainflag= args.trainflag, lr = args.lr, epochs=args.epochs, before_log_epochs = args.before_log_epochs,pretrained_model= args.premodel, combined=args.cd,fixed=args.fixed, mulobj=args.mulobj, add = args.add, quantile_flag= args.qt,checkpoint = args.checkpoint, noise= args.noise)
...
def parse_arguments():
    ...
    parser.add_argument('--premodel',type= str, default='LSA_SOS')
