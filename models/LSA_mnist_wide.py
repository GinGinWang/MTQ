...
class Encoder(BaseModule):
    ...
    def __init__(self, input_shape, code_length):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_features=reduce(mul, self.deepest_shape), out_features=1024),
            nn.BatchNorm1d(num_features=1024),
            activation_fn,
            nn.Linear(in_features=1024, out_features=code_length),
            nn.Sigmoid()
        )
...
class Decoder(BaseModule):
    ...
    def __init__(self, code_length, deepest_shape, output_shape):
        ...
        # FC network
        self.fc = nn.Sequential(
            nn.Linear(in_features=code_length, out_features=1024),
            nn.BatchNorm1d(num_features=1024),
            activation_fn,

        
            nn.Linear(in_features=1024, out_features=reduce(mul, deepest_shape)),
...
class LSA_MNIST_W(BaseModule):
    ...
    def __init__(self,  input_shape, code_length, num_blocks =1, hidden_size= 2048, est_name = None):
        ...
        """
        super(LSA_MNIST_W, self).__init__()

        self.input_shape = input_shape
        self.code_length = code_length
        self.est_name = est_name

        self.coder_name = 'LSAW'

        if est_name == None:  
            self.name = 'LSAW'
        else:
            self.name = f'LSAW_{est_name}'
