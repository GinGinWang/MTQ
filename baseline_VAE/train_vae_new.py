...
def load_binary_mnist(cfg, **kwcfg):
  ...
  dataset = MNIST(path='data/MNIST')
  trainset = dataset.train(cl)
  valset = dataset.val(cl)
  testset = dataset.test(cl)

  train_loader = DataLoader(trainset, batch_size = batch_size)
  val_loader = DataLoader(valset , batch_size = test_batch_size)
    ...
    for batch_idx, (x , y) in enumerate(train_data):
      ...
      if step % cfg.log_interval == 0:
        ...
        if valid_elbo > best_valid_elbo:
          ...
          torch.save(states, f'{cl}best_state_dict')
        else:
