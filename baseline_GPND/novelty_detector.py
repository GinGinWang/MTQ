...
def main(inliner_classes, total_classes = 10):
    ...
    with open(f'{dataset_name}_traindata.pkl', 'rb') as pkl:
        mnist_train = pickle.load(pkl)
    # with open('validdata_f.pkl', 'rb') as pkl:
    #     mnist_valid = pickle.load(pkl)
    with open(f'{dataset_name}_testdata.pkl', 'rb') as pkl:
    ...
    G.load_state_dict(torch.load(f"{dataset_name}_{cl}Gmodel.pkl"))
    E.load_state_dict(torch.load(f"{dataset_name}_{cl}Emodel.pkl"))
    ...
    def test(mnist_test):
        ...
        with open(os.path.join(f"{dataset_name}_results.txt"), "a") as file:
            file.write(
                "Class: %d\n AUC:%f\n\n" %
