...
for single_class in range(1):
	...
	for epoch in [1,2,3]:
		new_data ={}
		# train_data_name = f'{dataset}_gth_{single_class}_train.npz'
		...
		for i in range(lenth):
			...
			# print (new_data[i][0].shape)
			# print(new_data[i][1])
		# save(x,y) for every class
...
for single_class in range(1):
	...
	for data_class in range(10):
		...
		for epoch in [1,2,3]:
			featurelist =[]
			test_data_name = f'{dataset}_{epoch:02d}_m{single_class}_d{data_class}_test'
			for ft_class in range(72):
