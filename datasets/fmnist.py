...
class FMNIST(OneClassDataset):
    """
    Models FMNIST dataset for one class classification.
    """
    ...
    def val(self, normal_class):
        # type: (int) -> None
        """
        Sets FMNIST in validation mode.
    ...
    def train(self, normal_class, noise_ratio=0, noise2_ratio=0, noise3_ratio= 0, noise4_ratio =0):
        ...
        # fix the number of normal data, add noise from other dataset to the training set
            ...
            noise_split = datasets.MNIST('data/MNIST', train=True, download=False, transform=None)
...
#---------------------------------------------------------------------
    ...
    def __repr__(self):
        return ("ONE-CLASS FMNIST (normal class =  {} )").format(self.normal_class)